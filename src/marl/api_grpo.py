...
class APIGroupRelativePolicyOptimizer:
    ...
    def generate_response_samples(
        ...
        for i in range(k):
            try:
                ...
                logger.debug(
                    f"Generating sample {i+1}/{k} for agent {agent.role}"
                )
                ...
                sample_text = response.content[0].text
                samples.append(sample_text)

            except Exception as e:
                logger.error(
                    f"Error generating sample {i+1} for {agent.role}: {e}"
                )
                # Use fallback response
                samples.append(f"[Error generating response: {str(e)}]")
    ...
    def select_best_trajectory(
        ...
        if np.random.random() < exploration_rate:
            ...
            selected_idx = np.random.choice(len(advantages), p=probs)
            logger.debug(
    ...
    def update_agent_behaviors(
        ...
        logger.info(
            f"Updated {agent.role}: now has {len(agent.learned_behaviors)} "
            f"learned behaviors"
        )
