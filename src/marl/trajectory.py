...
class MultiTurnTrajectory:
    ...
    def __init__(self, max_turns: int = 15, task_description: str = ""):
        """
    ...
    def get_context_for_agent(
        ...
        if self.turns:
            ...
            recent_turns = self.turns[-max_history:] if len(self.turns) > max_history else self.turns

            for turn in recent_turns:
                context_parts.append(
                    f"Turn {turn.turn_number} | {turn.agent_role}: {turn.action}"
                )
        else:
...
class TrajectoryBeam:
    ...
    def prune(self) -> None:
        ...
        sorted_pairs = sorted(
            zip(self.scores, self.trajectories),
            key=lambda x: x[0],
            reverse=True
        )
    ...
    def get_best(self) -> Optional[MultiTurnTrajectory]:
