...
class CentralizedValueEstimator:
    ...
    def estimate_value(
        ...
        try:
            ...
            # Parse response
            scores = self._parse_evaluation_response(response.content[0].text)
    ...
    def _build_evaluation_prompt(
        ...
        prompt = f"""You are evaluating a multi-agent collaboration on a {task_type} task.
...
Be critical but fair. Use the full 0-10 range. Return ONLY the JSON object, no other text."""
    ...
    def compute_credit_assignment(
        ...
        # Get unique agents
        agent_ids = sorted(set(turn.agent_id for turn in trajectory.turns))
