...
from .api_grpo import APIGroupRelativePolicyOptimizer, Agent
...
class MARLTrainer:
    ...
    def __init__(
        ...
        self.k_samples = self.config.get('k_samples', 5)
        self.exploration_rate = self.config.get('exploration_rate', 0.1)
        self.learning_frequency = self.config.get('learning_frequency', 5)
        ...
        self.value_estimator = CentralizedValueEstimator(
            api_key=api_key,
        ...
        self.save_dir = Path(config.get('save_dir', 'runs')) / f"marl_{timestamp}"
    ...
    def train(
        ...
        for episode_num in pbar:
            ...
            trajectory, reward = self.run_episode_with_beam_search(
                episode_num + 1,
                verbose=verbose
            )
    ...
    def run_episode_with_beam_search(
        ...
        while turn < max_turns:
            ...
            for traj_idx, traj in enumerate(beam.get_trajectories()):
        ...
        if verbose:
            ...
            print(f"Components: Q={best_trajectory.reward_components.get('quality', 0):.1f}, "
    ...
    def _update_agent_behaviors(self, verbose: bool = False) -> None:
        ...
        if verbose:
            print(f"\n{'='*60}")
            print(f"Updating agent behaviors...")
            print(f"{'='*60}")
        ...
        for agent in self.agents:
            if agent.role in behaviors:
                ...
                new_behaviors = []
                for category, behavior_list in behaviors[agent.role].items():
                    new_behaviors.extend(behavior_list)

                self.grpo.update_agent_behaviors(
                    agent=agent,
    ...
    def _save_checkpoint(self, episode_num: int) -> None:
        ...
        checkpoint = {
            ...
            'agent_behaviors': {
                agent.role: len(agent.learned_behaviors)
                for agent in self.agents
            },
    ...
    def _get_training_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'final_10_avg': float(np.mean(rewards[-10:])) if len(rewards) >= 10 else float(np.mean(rewards)),
    ...
    def get_best_episode(self) -> Optional[MultiTurnTrajectory]:
