...
"""
    ...
    def is_done(self) -> bool:
        ...
        if self.history:
            ...
            if any(phrase in recent for phrase in ['final code', 'looks good', 'approved', 'complete']):
                return True
    ...
    def _evaluate_code_quality(self) -> float:
        ...
        if '->' in self.current_code or ': str' in self.current_code or ': int' in self.current_code:
            score += 1.0

        # Check for proper function definition
        if f"def {self.current_problem['requirements'][0].split('(')[0].replace('Function should be called ', '')}" in self.current_code:
            score += 1.0
        ...
        code_lines = len([l for l in self.current_code.split('\n') if l.strip()])
        if 5 <= code_lines <= 30:
            score += 1.0
    ...
    def _evaluate_collaboration(self) -> float:
        ...
        if any(word in history_text for word in ['previous', 'earlier', 'mentioned', 'suggested', 'building on']):
            score += 1.0
